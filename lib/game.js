(function(){
  ...
  Game.prototype.reviveAnimation = function (pos) {
    var reviveParticle = Asteroids.Particle.reviveParticle(pos, game);
  ...
  Game.prototype.fairPosition = function (pos) {
    for (var i = 0; i < this.asteroids.length; i++) {
      var dist = Math.sqrt(
        Math.pow(Math.abs(pos[0] - this.asteroids[i].pos[0]), 2) +
        Math.pow(Math.abs(pos[1] - this.asteroids[i].pos[1]), 2)
      );
