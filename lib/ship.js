(function(){
  ...
  Ship.prototype.powerEach = function (impulses) {
    var that = this;
    impulses.forEach()
  };
  ...
  Ship.prototype.throttleAgainst = function (vector) {
    var absVector = vector.slice().map(Math.abs);
    var throttleIndex = absVector.indexOf(0);
    this.vel[throttleIndex] *= 0.82;
  };

  Ship.prototype.throttleAllBut = function (vec) {
    this.vel.map(function (vec) {
      return vec * 0.4;
    });
  };
